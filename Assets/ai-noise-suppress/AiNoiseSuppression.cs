...
public class AiNoiseSuppression : AgoraUI
{
    internal AiNoiseSuppressionManager aiNoiseSuppressionManager;
    ...
    {
        // Create an instance of the AiNoiseSuppressionManager
        aiNoiseSuppressionManager = new AiNoiseSuppressionManager();

        // Setup UI elements
        SetupUI();

        // Attach a video surface to the local view game object.
        aiNoiseSuppressionManager.LocalView = LocalViewGo.AddComponent<VideoSurface>();
    }
    ...
    private void SetupUI()
    {
        ...
        if (aiNoiseSuppressionManager.configData.product != "Video Calling")
        {
        ...
        joinBtnGo.GetComponent<Button>().onClick.AddListener(aiNoiseSuppressionManager.Join);
        leaveBtnGo.GetComponent<Button>().onClick.AddListener(aiNoiseSuppressionManager.Leave);
        ...
        if (hostToggleGo && audienceToggleGo)
        {
            ...
            hostToggle.onValueChanged.AddListener((value) =>
            {
                ...
                {
                    audienceToggle.isOn = false;
                    aiNoiseSuppressionManager.SetClientRole("Host");
                }
            });
            audienceToggle.onValueChanged.AddListener((value) =>
            {
                ...
                {
                    hostToggle.isOn = false;
                    aiNoiseSuppressionManager.SetClientRole("Audience");
                }
            });

        }
        // Add a listener to the channel input field to update the channel name in AiNoiseSuppressionManager
    ...
    // Pass the channel name to the AiNoiseSuppressionManager class to fetch a token from the token server
    private void HandleChannelFieldChange(string newValue)
    {
        aiNoiseSuppressionManager.configData.channelName = newValue;
    }

    public override void OnDestroy()
    {
        base.OnDestroy();
        aiNoiseSuppressionManager.DestroyEngine();
        DestroyUIElements();
    }
